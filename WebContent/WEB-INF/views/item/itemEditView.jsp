					...
					<div>
							...
							<div class="form-row field-city">
								<div>
									<label for="id_city">City:</label> <input type="text"
										name="city" value="${item.city}" class="vTextField"
										maxlength="255" id="id_city" />
								</div>
							</div>
							<div class="form-row field-country">
								<div>
									<label for="id_country">Country:</label> <input type="text"
										name="country" value="${item.country}" class="vTextField"
										maxlength="255" id="id_country" />
								</div>
							</div>
							<div class="form-row field-option_value">
								<div>
									<label for="id_option_value">Option value:</label> <input
										type="number" name="option_value" value="${item.optionValue}"
										class="vIntegerField" id="id_option_value" />
								</div>
							</div>
							<div class="form-row field-price">
								<div>
									<label for="id_price">Price:</label> <input type="number"
										name="price" value="${item.price}" step="any" id="id_price" />
								</div>
							</div>
							<div class="form-row field-state">
								<div>
									<label for="id_state">State:</label> <input type="text"
										name="state" value="${item.state}" class="vTextField"
										maxlength="255" id="id_state" />
								</div>
							</div>
							<div class="form-row field-year">
								<div>
									<label for="id_year">Year:</label> <input type="text"
										name="year" value="${item.year}" class="vTextField"
										maxlength="255" id="id_year" />
								</div>
							</div>
									...
									<div class="related-widget-wrapper">
										<select name="id_order" id="id_order">
											...
											<c:forEach items="${orderList}" var="order_shown">
												<option value="${order_shown.orderId}" ${theOrder.orderId == order_shown.orderId ? 'selected' : '' }>Order${order_shown.orderId}:
						...
						<div class="submit-row">
							...
							<p class="deletelink-box">
								<a href="${pageContext.request.contextPath}/itemDelete?id=${item.itemid}"
